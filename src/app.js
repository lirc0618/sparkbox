...
function starterGuideTemplate(state) {
  ...
  return `<section class="starter-guide" aria-label="新用户启动流程">
    ...
    <div class="starter-steps">
      ...
      <button type="button" class="starter-step" data-action="new-bookmark"><span>2</span><strong>补充上下文</strong><small>${needsContext ? `${needsContext} 条需要摘录或备注` : "遇到登录墙就复制或转写可见内容"}</small></button>
...
function mobileCaptureAssistantTemplate() {
  return `<section class="mobile-capture-assistant" aria-label="移动采集助手">
    ...
    <div class="mobile-capture-actions">
      ...
      <button type="button" class="mobile-capture-card" data-action="capture-screenshot-bookmark">${icons.eye}<strong>截图转写</strong><span>先建收藏，再粘贴截图中的文字</span></button>
...
function inboxPage(state) {
  ...
  return `
    ...
    <form id="quick-capture-form" class="quick-capture">
      ...
      <p class="local-fallback">Sparkbox 会先确保原始内容不丢失；受限平台不会伪造摘要，补充摘录或截图转写后再让 AI 整理。</p>
    </form>
  ...
  return `<div class="modal-backdrop" data-action="close-modal"><section class="modal" role="dialog" aria-modal="true" aria-labelledby="modal-title" data-modal-panel>
    ...
    <form id="bookmark-form" data-id="${bookmark?.id || ""}">
      <div class="modal-body"><div class="form-grid">
        ...
        <div class="form-field full screenshot-tools"><div><strong>截图补充</strong><span>DeepSeek V4 暂不支持图片输入，请先复制或转写截图中的可见文字。</span></div></div>
...
function openScreenshotCapture() {
  ...
  else toast("已打开添加收藏，请把截图中的可见文字粘贴到摘录栏");
}
